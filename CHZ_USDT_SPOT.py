...
SOCKET = "wss://stream.binance.com:9443/ws/chzusdt@kline_1h" 
...
def on_message(ws, message): 
    ...
    candles_b = client.get_klines(symbol='CHZUSDT', interval=Client.KLINE_INTERVAL_1HOUR)
    ...
    resposta = requests.get('https://api.taapi.io/rsi?secret=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJjbHVlIjoiNjM3NTE4YjhmYzVhOGFkZmVjMWVhZWM4IiwiaWF0IjoxNjY4NjE4NDI0LCJleHAiOjMzMTczMDgyNDI0fQ.MuFuAO1q4s-UpYB_8CFGcivBglViDVveJzkTjJCei1c&exchange=binance&symbol=BTC/USDT&interval=1h')
   ...
   #definir_stop_loss   
    ...
    #versão_compra
    if (price_at > media2_rd) and (rsi_10 <= RSI_N):
    	print('<---BUY !!! SETUP 1')
    ...
    if (price_at < rd_a) and (rsi_10 <= RSI_BEAR):
    	print('<---BUY !!! SETUP 2')
    	order = client.order_market_buy(
    	symbol='CHZUSDT',
    	quantity=qt_btc_rd)
